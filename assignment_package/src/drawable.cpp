...
Drawable::Drawable(OpenGLContext* context)
    : m_count(-1), m_countT(-1), m_bufIdx(), m_bufPos(), m_bufNor(), m_bufCol(),
      m_bufInter(), m_bufInterT(),
      m_idxGenerated(false), m_posGenerated(false), m_norGenerated(false), m_colGenerated(false),
      m_interGenerated(false), m_interTGenerated(false),
      mp_context(context)
{}
...
void Drawable::destroyVBOdata()
{
    ...
    mp_context->glDeleteBuffers(1, &m_bufInterT);
    m_idxGenerated = m_posGenerated = m_norGenerated = m_colGenerated = m_interGenerated = m_interTGenerated = false;
    m_count = -1;
}
...
void Drawable::generateInterT() {
    m_interTGenerated = true;
    mp_context->glGenBuffers(1, &m_bufInterT);
}

void Drawable::generateIdxT()
{
    m_idxTGenerated = true;
    // Create a VBO on our GPU and store its handle in bufIdx
...
bool Drawable::bindInterT()
{
    if (m_interTGenerated) {
        mp_context->glBindBuffer(GL_ARRAY_BUFFER, m_bufInterT);
    }
    return m_interTGenerated;
}
